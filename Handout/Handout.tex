...
\end{equation}
where $X_1$ is defined by some initial conditions $\mu(x_1)$. Note how $f$ obeys the Markov property, so current state is dependent only on the previous state. Also note that $g$ shows that the current observation is only dependent on the current state.

% --------------------- Setting up the problem ---------------------
\section{Particle Filters}

Basically, the main idea is to use our observations of the system to determine the hidden state of the system. That is to say, we want to figure out $p(x_{n}|y_{1:n})$. Recognizing that this is a marginal is useful because then we can just say:
