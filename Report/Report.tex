...
% The prediction equation is just the update equation conditioned on $y_t$. To obtain an approximation of this equation we just need to reweight our update step by $g(y_t|x_t)$.

% \begin{equation}
% \widehat{p}(x_{1:t}|y_{1:t}) = \begin{cases}
% g(y_t|x_{t}^{(1)}) \quad \text{at} \ x_{1:t}^{(1)} \\
% \vdots \\
% g(y_t|x_{t}^{(N)}) \quad \text{at} \ x_{1:t}^{(N)} \\
% \end{cases}
% \end{equation}

% Notice that the distributions we are estimating are $p(x_{1:t}|y_{1:t-1})$ and $ p(x_{1:t}|y_{1:t})$, \textit{not} the transition distribution $f$.
