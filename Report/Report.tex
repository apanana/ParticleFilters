...
While we now have a nice equation, it would be really inefficient if we had to calculate this whole equation every single iteration of our system. Instead, we can formulate this recursively and see how some of the values carry through. We can show that:
...
We can then rewrite $p(x_t|y_{0:t-1})$ from the right side of \ref{want} using Bayes' rule:
\begin{equation} \label{sub}
p(x_t|y_{0:t-1}) = \frac{p(x_t,y_{0:t-1})}{p(y_{0:t-1})}
\end{equation}

Setting the right side of \ref{lhs} equal to the right side of both of these back into our initial equation for $p(x_t|y_{0:t})$ gives us:
\begin{equation}
\begin{split}
